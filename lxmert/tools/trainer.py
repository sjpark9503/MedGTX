...
from tools.data_collator import DataCollator, default_data_collator
...
class ElectraTrainer:
    ...
    def train(self, model_path: Optional[str] = None):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.logging_steps > 0 and self.global_step % self.args.logging_steps == 0) or (
                        ...
                        logs: Dict[str, float] = {}
                        logs["G_loss"] = (G_loss - G_logging_loss) / self.args.logging_steps
                        logs["D_loss"] = (D_loss - D_logging_loss) / self.args.logging_steps
        ...
        return TrainOutput(self.global_step, D_loss / self.global_step)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> float:
        ...
        if self.args.n_gpu > 1:
            ...
            G_loss = outputs[1][0].mean()
            D_loss = outputs[2][0].mean()
    ...
    def prediction_loop(
        ...
        G_eval_losses: List[float] = []
        D_eval_losses: List[float] = []
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            if loss is not None:
                G_eval_losses.append(loss[0])
                D_eval_losses.append(loss[1])
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            ...
            metrics = {}
        if len(G_eval_losses) > 0:
            metrics["G_eval_loss"] = np.mean(G_eval_losses)
        if len(D_eval_losses) > 0:
            metrics["D_eval_loss"] = np.mean(D_eval_losses)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            ...
            if has_labels:
                ...
                G_loss = outputs[1][0].mean().item()
                D_loss = outputs[2][0].mean().item()
                G_logit = outputs[1][1]
                D_logit = outputs[2][1]
            else:
                loss = None
        ...
        if prediction_loss_only:
            return ((G_loss,D_loss,), None, None)

        labels = inputs.get("labels")
        if labels is not None:
            labels = labels.detach()
        return ((G_loss,D_loss,), (G_logit.detach(),D_logit.detach(),), labels)
