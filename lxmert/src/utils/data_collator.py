...
class ErrorDetection_DataCollator:
    ...
    def __call__(self,features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
        if not isinstance(features[0], (dict, BatchEncoding)):
            features = [vars(f) for f in features]
        batch = self._tensorize_batch(features)
        masked_subs, kg_label_mask, kg_padding_mask = self.corruption(batch,corruption_type=task.split('_')[0])
        batch['kg_input_ids'] = masked_subs
        batch['kg_label_mask'] = kg_label_mask
        batch['kg_padding_mask'] = kg_padding_mask
    ...
    def corruption(self, batch: dict, corruption_type: str):
        """
        Corrupt the input.
        """
        assert corruption_type in ['both','deletion','replacement'], f"corruption strategy {corruption_type} not supported"
        if self.tokenizer.mask_token is None:
            raise ValueError(
                "This tokenizer does not have a mask token which is necessary for masked language modeling. Remove the --mlm flag if you want to use this tokenizer."
            )
        ignore_corruption = torch.eq(labels,-100)
        # We sample a few tokens in each sequence for masked-LM training (with probability args.mlm_probability defaults to 0.15 in Bert/RoBERTa)
        ...
        if corruption_type in ['both','replacement']:
            random_nodes = torch.randint(len(self.kg_special_token_ids),self.kg_size, inputs.shape, dtype=torch.long)
            inputs[indices_random] = random_nodes[indices_random]
        if corruption_type in ['both','deletion']:
            random_nodes = torch.randint(len(self.kg_special_token_ids),self.kg_size, inputs.shape, dtype=torch.long)
            inputs[indices_random] = random_nodes[indices_random]
